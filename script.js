...
const main = create('main', 'grid container',
    ...
    class Product{
        ...
        generateLayout(){
            this.product_code = create('span', 'product_code',"Код: "+ parseInt(productObj.code))
            this.product_status = create('div','product_status_tooltip_container', [
                create('span','product_status',  'Наличие')
            ])
            ...
            this.product_photo = create('div','product_photo',[
                create('a','url--link product__link',[ 
                    create('img',null,null,null,['src',productObj.primaryImageUrl.slice(0,-4) + '_220x220_1' + productObj.primaryImageUrl.slice(-4)],["width",'220px'],['height','220px'])],null,['href', '#'])
            ])

            this.description = create('div',"product_description")
                this.product__link = create('a', 'product__link', productObj.title, this.description, ['href', '#'])
                
            let assocProducts = productObj.assocProducts.replace("\n","").split(";")
...
console.log(' productObj.priceGold', productObj.priceGold);
this.product_price_club_card = create('p', 'product_price_club_card',[
    create('span','product_price_club_card_text', 'По карте<br>клуба'),
    create('span','goldPrice',(new Intl.NumberFormat('ru-RU').format(Math.round(parseFloat(productObj.priceGoldAlt) * 100)/100)).toString()+" ₽")
])

this.product_price_default = create('p', 'product_price_default',[
    create('span','retailPrice',  (new Intl.NumberFormat('ru-RU').format(Math.round(parseFloat(productObj.priceRetailAlt) * 100)/100)).toString()+" ₽")
])
...
this.product__wrapper = create('div', 'product__wrapper', [
    create('div','product_count_wrapper',[
        create('div', 'stepper', [
            create('input', "product__count stepper-input", null, null, ['type',"text"],['value',"1"]),
            create('span', "stepper-arrow up"),
            create('span', "stepper-arrow down")
        ])
    ]),
    create('span', "btn btn_cart", [
        create('svg', "ic ic_cart",[
        ...
        create('span', 'ng-binding', 'В корзину')
    ],null,[ 'data-url',"/cart/"], ['data-product-id',"9bf0afd7-5190-11e5-b9a9-00259036a192"])
])
 ...
 new Product(productObj).init().generateLayout();
